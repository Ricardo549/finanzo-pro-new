...
interface SimulatorsPageProps {
    ...
    isPro: boolean;
    language: string;
    onSaveGoal?: (goal: { amount: number, months: number }) => void;
}

export const SimulatorsPage: React.FC<SimulatorsPageProps> = ({ t, isPro, language, onSaveGoal }) => {
            ...
            {mode === 'compare' ? (
                <div className="space-y-6 animate-slide-up">
                    <Card className="bg-gradient-to-br from-indigo-600 to-indigo-800 text-white border-none relative overflow-hidden">
                        ...
                        <button
                            className="w-full mt-4 bg-white/20 hover:bg-white/30 text-white font-black py-3 rounded-xl uppercase text-xs tracking-widest transition-all active:scale-95 flex items-center justify-center gap-2"
                            onClick={() => onSaveGoal && onSaveGoal({ amount: parseFloat(goal), months: parseInt(months) })}
                        >
                            {language === 'pt-br' ? 'Salvar Meta na Home' : 'Save Goal to Home'} <ArrowRight size={14} />
                        </button>
                    </Card>
